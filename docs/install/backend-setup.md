...
Agent-owned files (`lib/supabase/`, `schema.sql`, `scripts/allow-admin.ts`, keep-alive workflow) already ship in the repo. You do **not** recreate them.
...
- **Hooks:** after you have run `schema.sql` **once**, attach `hook_before_user_created` as **Before User Created**. The SQL function does nothing until this is connected.

## 5. Run the schema (paste once)

Open `guestbook/schema.sql` (full schema including poem hearts). Paste **once** into the shared project's SQL Editor → Run. Do not run it twice.

`y2k-guestbook/schema.sql` is a slim copy without poetry tables — do not use it for the shared project if you need poem hearts.
...
**Local `.env.local` (both repos, gitignored):**
...
- `NEXT_PUBLIC_SUPABASE_URL`
- `NEXT_PUBLIC_SUPABASE_ANON_KEY`
- `SITE_URL` — live `https://` origin
- `DATABASE_URL` — after rate-limiting work

Do **not** put on Vercel: `SUPABASE_SERVICE_ROLE_KEY` unless you later add a server path that needs it.

## 8. Keep-alive (GitHub Actions)

Workflow: `.github/workflows/supabase-keep-alive.yml` in each repo. Set the two GitHub secrets above. Daily cron `15 12 * * *` + `workflow_dispatch`.

## 9. Vercel (linked GitHub)

For each Vercel project, set the Vercel list from section 7. Add the live origin to Google JS origins and Supabase Redirect URLs (`https://<your-domain>/auth/callback`).
