...
function shouldExtendStyledRun(items, more) {
  ...
  if (type === "a") {
    return (
      /^[a-z]$/.test(last) &&
      /^[a-z]$/.test(firstNew) &&
      firstNew.charCodeAt(0) === last.charCodeAt(0) + 1
    );
  }
...
function consumeStyledListRun(children, start) {
  ...
  // Last a./i. item ending in ":" + a fence (no later marker) still belongs
  // to that item — otherwise the fence becomes a sibling and loses indent.
  while (index < children.length && items.length > 0) {
    ...
    if (node.type === "code" && itemEndsWithColon(items[items.length - 1])) {
      items[items.length - 1].children.push(node);
      index += 1;
      continue;
    }
...
function applyDocsTransforms(tree) {
